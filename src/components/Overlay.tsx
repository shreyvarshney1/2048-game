...
const Overlay: React.FC = () => {
  ...
  if (victory) {
    return (
      <div className="z-999 absolute bottom-0 left-0 right-0 top-0 flex flex-col justify-center bg-[#EEE4DA] bg-opacity-80 text-center align-middle gap-2 rounded-md">
  ...
  if (defeat) {
    return (
      <div className="z-999 absolute bottom-0 left-0 right-0 top-0 flex flex-col justify-center bg-[#f08437] bg-opacity-50 text-center align-middle gap-2 rounded-md">
        <h1 className="text-2xl font-bold">Game over!</h1>
        <div>
          <button onClick={reset} className="opacity-100 p-3">
            Try again
          </button>
        </div>
      </div>
